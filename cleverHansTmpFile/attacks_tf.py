...
def jacobian(sess, x, grads, target, X, nb_features, nb_classes, feed=None):
    ...
    # each class would then have a <dy/dx>
...
def jacobian_graph(predictions, x, nb_classes):
    ...
    for class_ind in xrange(nb_classes):
        # class_ind w.r.t all classes, even class is correctly labelled...
        # get the derivatives out and add to list
        # a list of sums(dy/dx) for each x in xs... of data point

        # predictions: n x 10 shape,
        # selected predicions n x 1 ==> ys....  x ==> n x 784 by default
...
def jsma_batch(sess, x, pred, grads, X, theta, gamma, clip_min, clip_max,
    ...
    for ind, val in enumerate(X):
        ...
        if y_target is None:
            ...
            # model_argmax: computes the current model's prediction
        ...
        # jsma construction for each data input
